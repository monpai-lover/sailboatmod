...
public final class NationFlagUploadClient {
    public static Component uploadFromPath(String rawPath) {
        ...
        try {
            ...
            return queueUpload(source.displayName(), source.bytes());
    ...
    private static UploadSource readRemoteSource(String rawUrl) throws IOException {
        ...
        connection.setRequestProperty("User-Agent", "SailboatMod-FlagUploader/1.1.2");
    ...
    private static Component queueUpload(String displayName, byte[] bytes) {
        ...
        int chunkCount = Math.max(1, (bytes.length + NationFlagUploadService.CHUNK_SIZE - 1) / NationFlagUploadService.CHUNK_SIZE);
        for (int chunkIndex = 0; chunkIndex < chunkCount; chunkIndex++) {
            int start = chunkIndex * NationFlagUploadService.CHUNK_SIZE;
            int length = Math.min(NationFlagUploadService.CHUNK_SIZE, bytes.length - start);
            byte[] slice = new byte[length];
            System.arraycopy(bytes, start, slice, 0, length);
            ModNetwork.CHANNEL.sendToServer(new UploadNationFlagChunkPacket(uploadId, bytes.length, chunkIndex, chunkCount, slice));
        }
    ...
    private NationFlagUploadClient() {}
}
