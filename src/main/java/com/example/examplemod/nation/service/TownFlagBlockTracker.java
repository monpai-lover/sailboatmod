...
public final class TownFlagBlockTracker {
    ...
    public static void refreshTownFlags(MinecraftServer server, String townId) {
        if (server == null || townId == null || townId.isBlank()) {
            return;
        }
        for (ServerLevel level : server.getAllLevels()) {
            refreshTownFlags(level, townId);
        }
    }
    ...
    private static void refreshTownFlags(ServerLevel level, String townId) {
        Set<BlockPos> positions = LOADED_FLAGS.get(level.dimension());
        ...
        for (BlockPos pos : Set.copyOf(positions)) {
            if (!(level.getBlockEntity(pos) instanceof TownFlagBlockEntity blockEntity)) {
                stale.add(pos);
                continue;
            }
            if (townId.equals(blockEntity.getTownId())) {
