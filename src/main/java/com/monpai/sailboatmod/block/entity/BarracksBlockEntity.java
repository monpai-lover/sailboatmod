...
public class BarracksBlockEntity extends BlockEntity {
    ...
    private int secondaryColor = 0x8B0000;

    public BarracksBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntities.BARRACKS_BLOCK_ENTITY.get(), pos, state);
    }
    ...
    public void refreshFromTown() {
        ...
        if (town == null) { updateState("", "", 0x8B0000); return; }
        NationSavedData data = NationSavedData.get(level);
        NationRecord nation = town.nationId().isBlank() ? null : data.getNation(town.nationId());
        updateState(town.townId(), town.name(), nation == null ? 0x8B0000 : nation.secondaryColorRgb());
    }

    public static void serverTick(Level level, BlockPos pos, BlockState state, BarracksBlockEntity be) {
    ...
    @Override public void load(CompoundTag tag) {
        ...
        secondaryColor = tag.contains("SecondaryColor") ? tag.getInt("SecondaryColor") : 0x8B0000;
    }
